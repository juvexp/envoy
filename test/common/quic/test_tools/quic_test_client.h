...
#ifndef GFE_QUIC_TEST_TOOLS_QUIC_TEST_CLIENT_H_
#define GFE_QUIC_TEST_TOOLS_QUIC_TEST_CLIENT_H_
...
class QuicTestClient : public QuicSpdyStream::Visitor,
 ...
 public:
  ...
  ssize_t SendData(const string& data, bool last_data);
  ...
  void SendRequestsAndWaitForResponses(const std::vector<string>& url_list);
  ...
  ssize_t SendMessage(const gfe_spdy::SpdyHeaderBlock& headers,
  ...
  string SendCustomSynchronousRequest(const gfe_spdy::SpdyHeaderBlock& headers,
                                      const string& body);
  ...
  // Group 1.
  const gfe_spdy::SpdyHeaderBlock& response_trailers() const;
  bool response_complete() const;
  int64_t response_body_size() const;
  const string& response_body() const;
  // Group 2.
  bool response_headers_complete() const;
  const gfe_spdy::SpdyHeaderBlock* response_headers() const;
  const gfe_spdy::SpdyHeaderBlock* preliminary_headers() const;
  ...
  void set_priority(gfe_spdy::SpdyPriority priority) { priority_ = priority; }
  ...
  void OverrideSni(const string& sni) {
 ...
 private:
  class TestClientDataToResend : public QuicClient::QuicDataToResend {
   public:
    TestClientDataToResend(
        std::unique_ptr<gfe_spdy::SpdyHeaderBlock> headers,
        QuicStringPiece body,
        bool fin,
  ...
  struct PerStreamState {
    ...
    PerStreamState(QuicRstStreamErrorCode stream_error,
                   ...
                   const gfe_spdy::SpdyHeaderBlock& response_trailers,
                   uint64_t bytes_read,
    ...
    gfe_spdy::SpdyHeaderBlock preliminary_headers;
    string response;
  ...
  mutable gfe_spdy::SpdyHeaderBlock preliminary_headers_;
  mutable gfe_spdy::SpdyHeaderBlock response_headers_;

  // Parsed response trailers (if present), copied from the stream in OnClose.
  gfe_spdy::SpdyHeaderBlock response_trailers_;

  gfe_spdy::SpdyPriority priority_;
  string response_;
...
#endif  // GFE_QUIC_TEST_TOOLS_QUIC_TEST_CLIENT_H_
