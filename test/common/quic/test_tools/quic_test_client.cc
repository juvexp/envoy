...
ssize_t QuicTestClient::GetOrCreateStreamAndSendRequest(
  ...
  if (headers) {
    ...
    if (rv == QUIC_PENDING) {
      ...
      std::unique_ptr<gfe_spdy::SpdyHeaderBlock> new_headers(
          new gfe_spdy::SpdyHeaderBlock(headers->Clone()));
  ...
  if (headers != nullptr) {
    ...
    stream->WriteOrBufferBody(body.ToString(), fin, ack_listener);
    ret = body.length();
  }
  if (GetQuicReloadableFlag(enable_quic_stateless_reject_support)) {
    std::unique_ptr<gfe_spdy::SpdyHeaderBlock> new_headers;
    if (headers) {
      new_headers = QuicMakeUnique<gfe_spdy::SpdyHeaderBlock>(headers->Clone());
    }
...
ssize_t QuicTestClient::SendMessage(const gfe_spdy::SpdyHeaderBlock& headers,
                                    QuicStringPiece body) {
...
const gfe_spdy::SpdyHeaderBlock* QuicTestClient::response_headers() const {
  for (std::pair<QuicStreamId, QuicSpdyClientStream*> stream : open_streams_) {
    size_t bytes_read =
...
const gfe_spdy::SpdyHeaderBlock* QuicTestClient::preliminary_headers() const {
  for (std::pair<QuicStreamId, QuicSpdyClientStream*> stream : open_streams_) {
    size_t bytes_read =
        stream.second->stream_bytes_read() + stream.second->header_bytes_read();
    if (bytes_read > 0) {
...
const gfe_spdy::SpdyHeaderBlock& QuicTestClient::response_trailers() const {
