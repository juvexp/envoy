...
#include "gfe/http2/hpack/huffman/hpack_huffman_decoder.h"
...
TEST_F(GenericHuffmanTableTest, ValidateInternalsWithSmallCode) {
  ...
  SpdyStringPiece input(input_storage, ABSL_ARRAYSIZE(input_storage));
  // By symbol: (2) 00 (3) 010 (2) 00 (7) 10010 (4) 10000 (6 as pad) 1001100.
  char expect_storage[] = {0b00010001, 0b00101000, 0b01001100};
  SpdyStringPiece expect(expect_storage, ABSL_ARRAYSIZE(expect_storage));
...
class HpackHuffmanTableTest : public GenericHuffmanTableTest {
 protected:
  void SetUp() override {
    EXPECT_TRUE(table_.Initialize(kHpackHuffmanCode,
                                  ABSL_ARRAYSIZE(kHpackHuffmanCode)));
    EXPECT_TRUE(table_.IsInitialized());
  }

  // Use gfe_http2::HpackHuffmanDecoder for roundtrip tests.
  void DecodeString(const SpdyString& encoded, SpdyString* out) {
    gfe_http2::HpackHuffmanDecoder decoder;
    out->clear();
...
TEST_F(HpackHuffmanTableTest, RoundTripIndividualSymbols) {
  for (size_t i = 0; i != 256; i++) {
    ...
    SpdyStringPiece input(storage, ABSL_ARRAYSIZE(storage));
