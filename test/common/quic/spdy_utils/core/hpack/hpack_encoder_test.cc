...
class HpackEncoderPeer {
 public:
  ...
  // TODO: Remove or clean up these methods when deprecating
  ...
  static bool EncodeIncremental(HpackEncoder* encoder,
    ...
    encoderator->Next(random.Uniform(16), &output_buffer);
    while (encoderator->HasNext()) {
      SpdyString second_buffer;
      encoderator->Next(random.Uniform(16), &second_buffer);
