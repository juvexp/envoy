...
// Author: alyssar@google.com (Alyssa Wilk)
...
TEST(SpdyDataIRTest, Construct) {
  ...
  EXPECT_EQ(d1.data_len(), d1.flow_control_window_consumed());

  // Confirm copies a const string.
  const SpdyString foo = "foo";
  SpdyDataIR d3(/* stream_id = */ 3, foo);
  EXPECT_EQ(foo, d3.data());
  EXPECT_EQ(d3.data_len(), d3.flow_control_window_consumed());
