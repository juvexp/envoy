...
using ::testing::_;
using ::gfe_http2::Http2DecoderAdapter;
...
class SpdyFramerPeer {
 public:
  // TODO: Remove these methods when deprecating non-incremental
  ...
  static SpdySerializedFrame SerializeHeaders(SpdyFramer* framer,
    ...
    while (it.HasNextFrame()) {
      ...
      CHECK_GT(it.NextFrame(&frame_list_buffer), 0);
...
TEST_P(SpdyFramerTest, MultiValueHeader) {
  ...
  // TODO: If this pattern appears again, move to test class.
...
TEST_P(SpdyFramerTest, UnclosedStreamDataCompressorsOneByteAtATime) {
  ...
  SpdyDataIR data_ir(/* stream_id = */ 1,
                     SpdyStringPiece(bytes, ABSL_ARRAYSIZE(bytes)));
...
TEST_P(SpdyFramerTest, CreateWindowUpdate) {
...

  {
    ...
    CompareFrame(
        kDescription, frame, kH2FrameData, ABSL_ARRAYSIZE(kH2FrameData));
  }
...
TEST_P(SpdyFramerTest, MultipleContinuationFramesWithIterator) {
  ...
  EXPECT_GT(frame_it.NextFrame(&output_), 0);
...
TEST_P(SpdyFramerTest, ReadLargeSettingsFrame) {
  ...
  visitor.SimulateInFramer(
      ...
      control_frame.size());
  EXPECT_EQ(0, visitor.error_count_);
  EXPECT_EQ(3, static_cast<unsigned>(visitor.setting_count_));
  EXPECT_EQ(1, static_cast<unsigned>(visitor.settings_ack_sent_));
  ...
  EXPECT_EQ(0, visitor.error_count_);
  EXPECT_EQ(3 * 2, static_cast<unsigned>(visitor.setting_count_));
  EXPECT_EQ(2, static_cast<unsigned>(visitor.settings_ack_sent_));
}
...
TEST_P(SpdyFramerTest, ProcessDataFrameWithPadding) {
  ...
  CHECK_EQ(1, deframer_.ProcessInput(frame.data() + bytes_consumed, 1));
  ...
  CHECK_EQ(2, deframer_.ProcessInput(frame.data() + bytes_consumed, 2));
  CHECK_EQ(deframer_.state(), Http2DecoderAdapter::SPDY_FORWARD_STREAM_FRAME);
  CHECK_EQ(deframer_.spdy_framer_error(), Http2DecoderAdapter::SPDY_NO_ERROR);
  bytes_consumed += 2;
  ...
  CHECK_EQ(3, deframer_.ProcessInput(frame.data() + bytes_consumed, 3));
  ...
  CHECK_EQ(100, deframer_.ProcessInput(frame.data() + bytes_consumed, 100));
  CHECK_EQ(deframer_.state(), Http2DecoderAdapter::SPDY_CONSUME_PADDING);
  CHECK_EQ(deframer_.spdy_framer_error(), Http2DecoderAdapter::SPDY_NO_ERROR);
  bytes_consumed += 100;
  ...
  CHECK_EQ(18, deframer_.ProcessInput(frame.data() + bytes_consumed, 18));
...
TEST_P(SpdyFramerTest, ReadCompressedPushPromise) {
  ...
  visitor.SimulateInFramer(reinterpret_cast<unsigned char*>(frame.data()),
                           frame.size());
  EXPECT_EQ(42, visitor.last_push_promise_stream_);
  EXPECT_EQ(57, visitor.last_push_promise_promised_stream_);
...
TEST_P(SpdyFramerTest, ReadPushPromiseWithContinuation) {
  ...
  EXPECT_EQ(0, visitor.error_count_);
  EXPECT_EQ(1, visitor.last_push_promise_stream_);
  EXPECT_EQ(42, visitor.last_push_promise_promised_stream_);
  EXPECT_EQ(2, visitor.continuation_count_);
  EXPECT_EQ(0, visitor.zero_length_control_frame_header_data_count_);
  EXPECT_EQ(0, visitor.end_of_stream_count_);
...
TEST_P(SpdyFramerTest, ReceiveUnknownMidContinuation) {
  ...
  EXPECT_EQ(1, visitor.headers_frame_count_);
  EXPECT_EQ(0, visitor.continuation_count_);
  EXPECT_EQ(0, visitor.header_buffer_length_);
}
...
TEST_P(SpdyFramerTest, ReadUnknownExtensionFrame) {
  ...
  visitor.SimulateInFramer(
      ...
      control_frame.size());
  EXPECT_EQ(0, visitor.error_count_);
  EXPECT_EQ(1, static_cast<unsigned>(visitor.setting_count_));
  EXPECT_EQ(1, static_cast<unsigned>(visitor.settings_ack_sent_));
}
...
TEST_P(SpdyFramerTest, ReadUnknownExtensionFrameWithExtension) {
  ...
  visitor.SimulateInFramer(unknown_frame, ABSL_ARRAYSIZE(unknown_frame));
  EXPECT_EQ(0, visitor.error_count_);
  EXPECT_EQ(0x7fffffff, extension.stream_id_);
  EXPECT_EQ(20, extension.length_);
...
TEST_P(SpdyFramerTest, SizesTest) {
  EXPECT_EQ(9, kFrameHeaderSize);
  EXPECT_EQ(9, kDataFrameMinimumSize);
  EXPECT_EQ(9, kHeadersFrameMinimumSize);
  EXPECT_EQ(14, kPriorityFrameSize);
  EXPECT_EQ(13, kRstStreamFrameSize);
  EXPECT_EQ(9, kSettingsFrameMinimumSize);
  EXPECT_EQ(13, kPushPromiseFrameMinimumSize);
  EXPECT_EQ(17, kPingFrameSize);
  EXPECT_EQ(17, kGoawayFrameMinimumSize);
  EXPECT_EQ(13, kWindowUpdateFrameSize);
  EXPECT_EQ(9, kContinuationFrameMinimumSize);
  EXPECT_EQ(11, kGetAltSvcFrameMinimumSize);
  EXPECT_EQ(9, kFrameMinimumSize);

  EXPECT_EQ(16384, gfe_spdy::kHttp2DefaultFramePayloadLimit);
  EXPECT_EQ(16393, gfe_spdy::kHttp2DefaultFrameSizeLimit);
}
...
TEST_P(SpdyFramerTest, PushPromiseFrameFlags) {
  ...
  do {
    ...
    SpdySerializedFrame frame(SpdyFramerPeer::SerializePushPromise(
        &framer, push_promise, use_output_ ? &output_ : nullptr));
    // TODO: Add padding to SpdyPushPromiseIR,
...
// TODO: Add TEST_P(SpdyFramerTest, AltSvcFrameFlags)
...
TEST_P(SpdyFramerTest, ReadChunkedAltSvcFrame) {
  ...
  EXPECT_EQ(0, visitor.error_count_);
  EXPECT_EQ(1, static_cast<unsigned>(visitor.altsvc_count_));
  ASSERT_NE(nullptr, visitor.test_altsvc_ir_);
  ASSERT_EQ(2, visitor.test_altsvc_ir_->altsvc_vector().size());
...
TEST_P(SpdyFramerTest, ProcessAllInput) {
  ...
  EXPECT_EQ(1, visitor->headers_frame_count_);
  EXPECT_EQ(1, visitor->data_frame_count_);
  EXPECT_EQ(strlen(four_score), visitor->data_bytes_);
}
...
TEST_P(SpdyFramerTest, ProcessAtMostOneFrame) {
  ...
  for (int first_size = 0; first_size <= buf_size; ++first_size) {
...
TEST_P(SpdyFramerTest, SpdyFrameIRSize) {
  ...
  SpdyDataIR data_ir(1, SpdyStringPiece(bytes, ABSL_ARRAYSIZE(bytes)));
  ...
  headers_ir_with_continuation.SetHeader("alpha", string(100000, 'x'));
  headers_ir_with_continuation.SetHeader("beta", string(100000, 'x'));
  ...
  push_promise_ir.SetHeader("alpha", string(100000, 'x'));
  push_promise_ir.SetHeader("beta", string(100000, 'x'));
