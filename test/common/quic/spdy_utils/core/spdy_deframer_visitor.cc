...
// TODO: Switch to using //gfe/http2/http2_constants.h when ready.
enum Http2FrameType {
  DATA = 0,
  HEADERS = 1,
  PRIORITY = 2,
...
void SpdyTestDeframerImpl::AtGoAwayEnd() {
  ...
  if (CHECK_NOTNULL(goaway_description_)->empty()) {
    ...
    CHECK_EQ(0, goaway_description_->size());
  }
...
void SpdyTestDeframerImpl::OnStreamPadding(SpdyStreamId stream_id, size_t len) {
  ...
  CHECK_GE(255, len);
  padding_len_ += len;
  CHECK_LE(padding_len_, 256) << "len=" << len;
}
...
class LoggingSpdyDeframerDelegate : public SpdyDeframerVisitorInterface {
 public:
  ...
  // The SpdyFramer will not process any more data at this point.
  void OnError(gfe_http2::Http2DecoderAdapter::SpdyFramerError error,
