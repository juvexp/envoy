...
#ifndef GFE_QUIC_HTTP_DECODER_PAYLOAD_DECODERS_QUIC_HTTP_PAYLOAD_DECODER_BASE_TEST_UTIL_H_
#define GFE_QUIC_HTTP_DECODER_PAYLOAD_DECODERS_QUIC_HTTP_PAYLOAD_DECODER_BASE_TEST_UTIL_H_
...
class QuicHttpPayloadDecoderBaseTest : public QuicHttpRandomDecoderTest {
 protected:
  ...
  // Randomize the payload decoder, sets the payload decoder's
  // frame_header_, then start decoding the payload. Called by
  // QuicHttpRandomDecoderTest. This method is final so that we can
  // always perform certain actions when QuicHttpRandomDecoderTest
  // starts the decoding of a payload, such as randomizing the payload
  // decoder, injecting the frame header and counting fast decoding
  // cases. Sub-classes must implement StartDecodingPayload to perform
  // their initial decoding of a frame's payload.
  ...
  // TODO: Add helper method for verifying these are both non-zero,
...
#endif  // GFE_QUIC_HTTP_DECODER_PAYLOAD_DECODERS_QUIC_HTTP_PAYLOAD_DECODER_BASE_TEST_UTIL_H_
