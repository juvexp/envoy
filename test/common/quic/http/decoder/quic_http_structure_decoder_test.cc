...
// TODO: Create type parameterized tests for
...
class QuicHttpStructureDecoderTest : public QuicHttpRandomDecoderTest {
 protected:
  ...
  AssertionResult DecodeLeadingStructure(const S* expected,
    ...
    if (expected != nullptr) {
      LOG_FIRST_N(INFO, 5) << "DecodeLeadingStructure expected: " << *expected;
      LOG_FIRST_N(INFO, 5) << "DecodeLeadingStructure   actual: " << structure_;
      VERIFY_EQ(*expected, structure_);
    }
    return AssertionSuccess();
  }
  ...
  AssertionResult TestDecodingRandomizedStructures() {
    ...
    VERIFY_LT(count, 1000 * 1000) << "That should be plenty!";
    VERIFY_SUCCESS(TestDecodingRandomizedStructures(count));
    return AssertionSuccess();
  }
