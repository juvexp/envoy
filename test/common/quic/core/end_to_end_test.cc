...
#include "gfe/gfe2/jetstream/http_constants.h"
#include "gfe/gfe2/test_tools/valgrind_utils.h"
...
#include "test/common/quic/test_tools/packet_dropping_test_writer.h"
...
#include "test/common/quic/test_tools/quic_session_peer.h"
#include "test/common/quic/test_tools/quic_spdy_session_peer.h"
#include "test/common/quic/test_tools/quic_stream_peer.h"
...
class ClientDelegate : public PacketDroppingTestWriter::Delegate {
 public:
  ...
  void OnCanWrite() override {
    gfe2::EpollEvent event(EPOLLOUT);
...
class EndToEndTest : public QuicTestWithParam<TestParams> {
 protected:
  EndToEndTest()
    ...
    FLAGS_gfe2_reloadable_flag_quic_store_version_before_signalling = true;
  ...
  bool Initialize() {
    ...
    // TODO: Consider setting the congestion control algorithm for the
    ...
    static gfe2::EpollEvent event(EPOLLOUT);
  ...
  void VerifyCleanConnection(bool had_packet_loss) {
    ...
    // TODO: Determine why this becomes even more flaky with BBR
    // enabled.  b/62141144
    ...
    // TODO: Restore the check for packets_dropped equals 0.
...
class EndToEndTestWithTls : public EndToEndTest {
 protected:
  EndToEndTestWithTls() : EndToEndTest() {
    FLAGS_gfe2_reloadable_flag_delay_quic_server_handshaker_construction = true;
...
TEST_P(EndToEndTest, LargePostWithPacketLoss) {
  if (!BothSidesSupportStatelessRejects()) {
    ...
    // brutal.
    // TODO: Until we support redundant SREJ packets, don't
...
// TODO: Needs to get turned back to EndToEndTestWithTls
...
TEST_P(EndToEndTest, ConnectionMigrationClientPortChanged) {
  ...
  gfe2::EpollServer* eps = client_->epoll_server();
...
TEST_P(EndToEndTest, FlowControlsSynced) {
  ...
  QuicDispatcher* dispatcher =
      QuicServerPeer::GetDispatcher(server_thread_->server());
  auto server_session = down_cast<QuicSpdySession*>(
  ...
  ExpectFlowControlsSynced(
      ...
      QuicSessionPeer::GetMutableCryptoStream(server_session)
          ->flow_controller());
  gfe_spdy::SpdyFramer spdy_framer(gfe_spdy::SpdyFramer::ENABLE_COMPRESSION);
  gfe_spdy::SpdySettingsIR settings_frame;
  settings_frame.AddSetting(gfe_spdy::SETTINGS_MAX_HEADER_LIST_SIZE,
                            kDefaultMaxUncompressedHeaderSize);
  gfe_spdy::SpdySerializedFrame frame(
      spdy_framer.SerializeFrame(settings_frame));
  ...
  if (GetQuicReloadableFlag(quic_send_max_header_list_size)) {
    ...
    // receive the frame.
    // TODO: Rewrite this part because it is hacky.
...
TEST_P(EndToEndTestWithTls, BadPacketHeaderTruncated) {
  ...
  util::Clock::RealClock()->Sleep(absl::Milliseconds(100));
  // Pause the server so we can access the server's internals without races.
  server_thread_->Pause();
  QuicDispatcher* dispatcher =
      QuicServerPeer::GetDispatcher(server_thread_->server());
  ...
  void AddRequestAndResponseWithServerPush(QuicString host,
    ...
    for (size_t i = 0; i < num_resources; ++i) {
      ...
      push_resources.push_back(QuicHttpResponseCache::ServerPushInfo(
          resource_url, std::move(response_headers),
          gfe_spdy::kV3LowestPriority, body));
    }
...
// TODO: this test seems to cause net_unittests timeouts :|
...
TEST_P(EndToEndTest, DISABLED_TestHugeResponseWithPacketLoss) {
  ...
  client_->WaitForResponse();
  // TODO: Fix this test to work with stateless rejects.
  if (!BothSidesSupportStatelessRejects()) {
    VerifyCleanConnection(true);
  }
}
