...
#include "common/quic/core/quic_data_writer.h"
...
using gfe_spdy::ERROR_CODE_PROTOCOL_ERROR;
using gfe_spdy::SETTINGS_ENABLE_PUSH;
using gfe_spdy::SETTINGS_HEADER_TABLE_SIZE;
using gfe_spdy::SETTINGS_INITIAL_WINDOW_SIZE;
using gfe_spdy::SETTINGS_MAX_CONCURRENT_STREAMS;
using gfe_spdy::SETTINGS_MAX_FRAME_SIZE;
using gfe_spdy::SETTINGS_MAX_HEADER_LIST_SIZE;
using gfe_spdy::Spdy3PriorityToHttp2Weight;
using gfe_spdy::SpdyAltSvcWireFormat;
using gfe_spdy::SpdyDataIR;
using gfe_spdy::SpdyErrorCode;
using gfe_spdy::SpdyFramer;
using gfe_spdy::SpdyFramerVisitorInterface;
using gfe_spdy::SpdyGoAwayIR;
using gfe_spdy::SpdyHeaderBlock;
using gfe_spdy::SpdyHeadersHandlerInterface;
...
class QuicHeadersStreamTest : public QuicTestWithParam<TestParams> {
 public:
  ...
  QuicHeadersStreamTest()
    ...
    deframer_ = std::unique_ptr<gfe_http2::Http2DecoderAdapter>(
        new gfe_http2::Http2DecoderAdapter());
  ...
  void WriteHeadersAndCheckData(QuicStreamId stream_id,
    ...
    EXPECT_FALSE(deframer_->HasError())
        << gfe_http2::Http2DecoderAdapter::SpdyFramerErrorToString(
  ...
  QuicTransportVersion transport_version() const {
    return test_params_.version.transport_version;
  }

  ParsedQuicVersionVector GetVersion() {
    ParsedQuicVersionVector versions;
    versions.push_back(test_params_.version);
    return versions;
  }
  ...
  std::unique_ptr<gfe_http2::Http2DecoderAdapter> deframer_;
...
TEST_P(QuicHeadersStreamTest, ProcessPriorityFrame) {
  ...
  for (SpdyPriority priority = 0; priority < 7; ++priority) {
    for (QuicStreamId stream_id = client_id_1_; stream_id < client_id_3_;
      ...
      gfe_spdy::SpdyPriorityIR priority_frame(stream_id, parent_stream_id,
                                              weight, true);
