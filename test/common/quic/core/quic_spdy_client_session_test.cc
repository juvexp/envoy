...
#include "test/common/quic/test_tools/mock_quic_spdy_client_stream.h"
 ...
 protected:
  ...
  void CompleteCryptoHandshake(uint32_t server_max_incoming_streams) {
    ...
    QuicCryptoClientStream* stream =
        down_cast<QuicCryptoClientStream*>(session_->GetMutableCryptoStream());
...
TEST_P(QuicSpdyClientSessionTest, InvalidPacketReceived) {
  ...
  EXPECT_CALL(*connection_, ProcessUdpPacket(server_address, client_address, _))
      .WillRepeatedly(
          Invoke(absl::implicit_cast<MockQuicConnection*>(connection_),
