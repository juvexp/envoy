...
TEST_F(QuicStreamSequencerBufferTest, ReadvError) {
  QuicString source = QuicString(100, 'b');
  clock_.AdvanceTime(QuicTime::Delta::FromSeconds(1));
  QuicTime t1 = clock_.ApproximateNow();
  // Write something into [0, 100).
  size_t written;
  ...
  EXPECT_EQ(0, error_details_.find(QuicStrCat(
                   "QuicStreamSequencerBuffer error: Readv() dest == nullptr: ",
                   true, " blocks_[0] == nullptr: ", false)));
}
