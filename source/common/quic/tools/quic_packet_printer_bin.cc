...
int main(int argc, char* argv[]) {
  ...
  string perspective_string = argv[1];
  gfe_quic::Perspective perspective;
  if (perspective_string == "client") {
    perspective = gfe_quic::Perspective::IS_CLIENT;
  } else if (perspective_string == "server") {
    perspective = gfe_quic::Perspective::IS_SERVER;
  } else {
    std::cerr << "Invalid perspective.  Usage: " << argv[0]
              << " client|server <hex>\n";
    return 1;
  }
  string hex = gfe_quic::QuicTextUtils::HexDecode(argv[2]);
  gfe_quic::ParsedQuicVersionVector versions = gfe_quic::AllSupportedVersions();
  // Fake a time since we're not actually generating acks.
  gfe_quic::QuicTime start(gfe_quic::QuicTime::Zero());
  gfe_quic::QuicFramer framer(versions, start, perspective);
  if (!FLAGS_quic_version.empty()) {
    for (gfe_quic::ParsedQuicVersion version : versions) {
      if (gfe_quic::QuicVersionToString(version.transport_version) ==
          FLAGS_quic_version) {
        framer.set_version(version);
      }
    }
  }
  gfe_quic::QuicPacketPrinter visitor(&framer);
  framer.set_visitor(&visitor);
  gfe_quic::QuicEncryptedPacket encrypted(hex.c_str(), hex.length());
