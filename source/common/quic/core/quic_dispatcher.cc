...
#include "common/quic/core/stateless_rejector.h"
...
class StatelessConnectionTerminator {
 public:
  ...
  void CloseConnection(QuicErrorCode error_code,
    ...
    // TODO: Use the right long header type for conneciton close sent by
    // dispatcher.
...
void QuicDispatcher::ProcessPacket(const QuicSocketAddress& self_address,
  ...
  // TODO: Return a status describing if/why a packet was dropped,
  //                and log somehow.  Maybe expose as a varz.
  // TODO: Consider invalidate the current_* variables so processing of the
...
QuicDispatcher::QuicPacketFate QuicDispatcher::ValidityChecks(
  ...
  if (GetQuicRestartFlag(quic_enable_accept_random_ipn)) {
    QUIC_FLAG_COUNT_N(gfe2_restart_flag_quic_enable_accept_random_ipn, 1, 2);
...
bool QuicDispatcher::IsValidStatelessResetToken(absl::uint128 token) const {
  DCHECK(false);
  return false;
}
...
void QuicDispatcher::MaybeRejectStatelessly(QuicConnectionId connection_id,
  ...
  if (version.handshake_protocol == PROTOCOL_TLS1_3) {
    ...
    return;
    // TODO: Support buffering non-ClientHello packets when using TLS.
  }
  // TODO: This logic should probably live completely inside the rejector.
  if (!FLAGS_quic_allow_chlo_buffering ||
