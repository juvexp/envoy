...
size_t QuicSpdyStream::WriteHeaders(
  ...
  if (fin) {
    // TODO: Add test to ensure fin_sent_ is set whenever a fin is sent.
    set_fin_sent(true);
    CloseWriteSide();
  }
...
void QuicSpdyStream::OnStreamHeaderList(bool fin,
  ...
  // be reset.
  // TODO: Use an explicit "headers too large" signal. An empty header list
...
bool QuicSpdyStream::ParseHeaderStatusCode(const SpdyHeaderBlock& header,
  ...
  SpdyHeaderBlock::const_iterator it =
      header.find(gfe_spdy::kHttp2StatusHeader);
  if (it == header.end()) {
    return false;
  }
