...
using gfe_spdy::Http2WeightToSpdy3Priority;
using gfe_spdy::Spdy3PriorityToHttp2Weight;
using gfe_spdy::SpdyErrorCode;
using gfe_spdy::SpdyFramer;
using gfe_spdy::SpdyFramerDebugVisitorInterface;
using gfe_spdy::SpdyFramerVisitorInterface;
using gfe_spdy::SpdyFrameType;
using gfe_spdy::SpdyHeadersIR;
using gfe_spdy::SpdyKnownSettingsId;
using gfe_spdy::SpdyPingId;
using gfe_spdy::SpdyPriority;
using gfe_spdy::SpdyPushPromiseIR;
using gfe_spdy::SpdySerializedFrame;
using gfe_spdy::SpdySettingsId;
using gfe_spdy::SpdyStreamId;
...
class QuicSpdySession::SpdyFramerVisitor
 ...
 public:
  ...
  gfe_spdy::SpdyHeadersHandlerInterface* OnHeaderFrameStart(
  ...
  void OnError(gfe_http2::Http2DecoderAdapter::SpdyFramerError error) override {
    ...
    switch (error) {
      case gfe_http2::Http2DecoderAdapter::SpdyFramerError::
    ...
    CloseConnection(
        QuicStrCat(
            "SPDY framing error: ",
            gfe_http2::Http2DecoderAdapter::SpdyFramerErrorToString(error)),
        code);
  }
  ...
  void OnSettingOld(SpdyKnownSettingsId id, uint32_t value) override {
    ...
    switch (id) {
      case gfe_spdy::SETTINGS_HEADER_TABLE_SIZE:
        session_->UpdateHeaderEncoderTableSize(value);
        break;
      case gfe_spdy::SETTINGS_ENABLE_PUSH:
        ...
        break;
      // TODO: Need to support SETTINGS_MAX_HEADER_LIST_SIZE when
      // clients are actually sending it.
      case gfe_spdy::SETTINGS_MAX_HEADER_LIST_SIZE:
  ...
  void OnHeaders(SpdyStreamId stream_id,
    ...
    // TODO: avoid down-conversion and plumb SpdyStreamPrecedence through
    // QuicHeadersStream.
  ...
  void OnPriority(SpdyStreamId stream_id,
    ...
    // TODO : implement real HTTP/2 weights and dependencies instead of
...
size_t QuicSpdySession::WritePriority(QuicStreamId id,
  ...
  gfe_spdy::SpdyPriorityIR priority_frame(id, parent_stream_id, weight,
                                          exclusive);
...
size_t QuicSpdySession::SendMaxHeaderListSize(size_t value) {
  ...
  settings_frame.AddSetting(gfe_spdy::SETTINGS_MAX_HEADER_LIST_SIZE, value);
...
// TODO : Why is SpdyStreamId used instead of QuicStreamId?
