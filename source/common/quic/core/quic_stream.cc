...
void QuicStream::OnStreamFrame(const QuicStreamFrame& frame) {
  ...
  if (GetQuicReloadableFlag(quic_stream_too_long) && is_stream_too_long) {
    ...
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_stream_too_long, 4, 5);
    QUIC_PEER_BUG
...
void QuicStream::OnStreamReset(const QuicRstStreamFrame& frame) {
  ...
  if (GetQuicReloadableFlag(quic_stream_too_long) &&
    ...
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_stream_too_long, 5, 5);
...
void QuicStream::WriteOrBufferData(
  ...
  if (data.length() > 0) {
    ...
    if (GetQuicReloadableFlag(quic_stream_too_long) &&
      ...
      QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_stream_too_long, 1, 5);
  ...
  if (CanWriteNewData() || span.empty()) {
    ...
    if (!span.empty()) {
      ...
      if (GetQuicReloadableFlag(quic_stream_too_long) &&
        ...
        QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_stream_too_long, 3, 5);
        QUIC_BUG << "Write too many data via stream " << id_;
        CloseConnectionWithDetails(
            QUIC_STREAM_LENGTH_OVERFLOW,
            absl::StrCat("Write too many data via stream ", id_));
