...
bool TlsClientHandshaker::CryptoConnect() {
  ...
  // TODO: This only verifies certs on initial connection, not on
...
int TlsClientHandshaker::num_sent_client_hellos() const {
  // TODO: Return a sensible value here.
  return 0;
}
...
QuicLongHeaderType TlsClientHandshaker::GetLongHeaderType(
  ...
  // TODO: Returns the right header type when actually using TLS
  // handshaker.
...
void TlsClientHandshaker::AdvanceHandshake() {
  ...
  if (state_ == STATE_HANDSHAKE_COMPLETE) {
    // TODO: Handle post-handshake messages.
    return;
  }
  ...
  if (should_close) {
    // TODO: Surface error details from the error queue when ssl_error
    // is SSL_ERROR_SSL.
...
void TlsClientHandshaker::CloseConnection() {
  // TODO: Instead of QUIC_HANDSHAKE_FAILED, this should be
...
enum ssl_verify_result_t TlsClientHandshaker::VerifyCert(uint8_t* out_alert) {
  ...
  // TODO: Pass the CRYPTO_BUFFERs into the QUIC stack to avoid copies.
