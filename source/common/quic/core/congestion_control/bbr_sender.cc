...
void BbrSender::SetFromConfig(const QuicConfig& config,
  ...
  if (GetQuicReloadableFlag(quic_bbr_less_probe_rtt) &&
    ...
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_bbr_less_probe_rtt, 1, 3);
    probe_rtt_based_on_bdp_ = true;
  }
  if (GetQuicReloadableFlag(quic_bbr_less_probe_rtt) &&
      config.HasClientRequestedIndependentOption(kBBR7, perspective)) {
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_bbr_less_probe_rtt, 2, 3);
    probe_rtt_skipped_if_similar_rtt_ = true;
  }
  if (GetQuicReloadableFlag(quic_bbr_less_probe_rtt) &&
      config.HasClientRequestedIndependentOption(kBBR8, perspective)) {
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_bbr_less_probe_rtt, 3, 3);
  ...
  if (GetQuicReloadableFlag(quic_one_tlp) &&
    ...
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_one_tlp, 2, 2);
...
// TODO: Move this logic into BandwidthSampler.
...
void BbrSender::CalculatePacingRate() {
  ...
  if (rate_based_recovery_ && InRecovery()) {
    QUIC_FLAG_COUNT(gfe2_reloadable_flag_quic_bbr_rate_recovery);
