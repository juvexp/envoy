...
#include "common/quic/platform/api/quic_aligned.h"
...
// TODO when we get real IETF QUIC, need to get
...
QuicFramer::QuicFramer(const ParsedQuicVersionVector& supported_versions,
  ...
  if (use_incremental_ack_processing_) {
    QUIC_FLAG_COUNT(gfe2_reloadable_flag_quic_use_incremental_ack_processing3);
  }
...
// TODO: Change this method to take a ParsedQuicVersion.
...
std::unique_ptr<QuicEncryptedPacket> QuicFramer::BuildVersionNegotiationPacket(
  ...
  uint8_t flags = static_cast<uint8_t>(
      ...
      // TODO: Remove this QUIC_VERSION_32 is retired.
...
bool QuicFramer::ProcessVersionNegotiationPacket(
  ...
  do {
    ...
    // TODO: Use ReadUInt32() once QUIC_VERSION_38 and earlier
    // are removed.
...
bool QuicFramer::ProcessPublicResetPacket(QuicDataReader* reader,
  ...
  // TODO: validate nonce to protect against DoS.
...
bool QuicFramer::ProcessAndCalculatePacketNumber(
  ...
  // TODO: Explore the usefulness of trying multiple packet numbers
  ...
  while (!reader->IsDoneReading()) {
    ...
    if (frame_type & kQuicFrameTypeSpecialMask) {
      ...
      if ((version_.transport_version != QUIC_VERSION_41 &&
        ...
        // TODO: Remove frame when deprecating
        // gfe2_reloadable_flag_quic_use_incremental_ack_processing3.
        QuicAckFrame frame;
...
bool QuicFramer::ProcessStreamFrame(QuicDataReader* reader,
  ...
  // TODO: Don't use QuicStringPiece as an intermediary.
...
QuicStringPiece QuicFramer::GetAssociatedDataFromEncryptedPacket(
    ...
    QuicPacketNumberLength packet_number_length) {
  // TODO: This is identical to QuicData::AssociatedData.
...
bool QuicFramer::AppendIetfAckFrame(const QuicAckFrame& frame,
  ...
  // TODO: Use the shift from TLS transport parameters.
  ...
  // TODO: Allow this code to write fewer ack blocks when it runs
