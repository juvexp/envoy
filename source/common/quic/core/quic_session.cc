...
QuicSession::QuicSession(QuicConnection* connection,
  ...
  if (use_control_frame_manager()) {
    QUIC_FLAG_COUNT(gfe2_reloadable_flag_quic_use_control_frame_manager);
  }
...
void QuicSession::OnStreamFrame(const QuicStreamFrame& frame) {
  // TODO deal with the error case of stream id 0.
...
void QuicSession::OnBlockedFrame(const QuicBlockedFrame& frame) {
  // TODO: Compare our flow control receive windows for specified
...
void QuicSession::OnCryptoHandshakeEvent(CryptoHandshakeEvent event) {
  switch (event) {
    // TODO: Move the logic of setting the encrypter/decrypter
