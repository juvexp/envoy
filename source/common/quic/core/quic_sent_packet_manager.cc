...
#include "common/quic/core/congestion_control/pacing_sender.h"
...
QuicSentPacketManager::QuicSentPacketManager(
  ...
  QUIC_FLAG_COUNT(gfe2_reloadable_flag_quic_better_crypto_retransmission);
...
void QuicSentPacketManager::SetFromConfig(const QuicConfig& config) {
  ...
  if (GetQuicReloadableFlag(quic_max_ack_delay) &&
    ...
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_max_ack_delay, 1, 2);
    rtt_stats_.set_ignore_max_ack_delay(true);
  }
  if (GetQuicReloadableFlag(quic_max_ack_delay) &&
      config.HasClientSentConnectionOption(kMAD1, perspective_)) {
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_max_ack_delay, 2, 2);
  ...
  if (GetQuicReloadableFlag(quic_max_ack_delay2) &&
    ...
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_max_ack_delay2, 1, 4);
    min_tlp_timeout_ = QuicTime::Delta::Zero();
  }
  if (GetQuicReloadableFlag(quic_max_ack_delay2) &&
      config.HasClientSentConnectionOption(kMAD3, perspective_)) {
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_max_ack_delay2, 2, 4);
  ...
  if (GetQuicReloadableFlag(quic_one_tlp) &&
    ...
    QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_one_tlp, 1, 2);
...
void QuicSentPacketManager::MarkForRetransmission(
  ...
  if (session_decides_what_to_write()) {
    if (transmission_type == HANDSHAKE_RETRANSMISSION ||
        ...
        transmission_type == PROBING_RETRANSMISSION ||
        transmission_type == RTO_RETRANSMISSION) {
      // TODO: Consider to make RTO and PROBING retransmission
  ...
  // TODO: Currently the RTO can fire while there are pending NACK
...
void QuicSentPacketManager::MarkPacketHandled(QuicPacketNumber packet_number,
  ...
  if (newest_transmission == packet_number) {
    ...
    // TODO: Instead of handling all crypto packets special,
...
const QuicTime QuicSentPacketManager::GetRetransmissionTime() const {
  ...
  switch (GetRetransmissionMode()) {
    ...
    case TLP_MODE: {
      // TODO: When CWND is available, it would be preferable to
...
QuicBandwidth QuicSentPacketManager::BandwidthEstimate() const {
  // TODO: Remove BandwidthEstimate from SendAlgorithmInterface
...
bool QuicSentPacketManager::OnAckFrameEnd(QuicTime ack_receive_time) {
  ...
  // TODO: Move this line to PostProcessAfterMarkingPacketHandled
  // when deprecating gfe2_reloadable_flag_quic_use_incremental_ack_processing3.
