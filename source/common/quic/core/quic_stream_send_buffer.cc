...
QuicStreamSendBuffer::QuicStreamSendBuffer(QuicBufferAllocator* allocator)
  ...
  if (free_mem_slice_out_of_order_) {
    QUIC_FLAG_COUNT(gfe2_reloadable_flag_quic_free_mem_slice_out_of_order);
  }
  if (enable_fast_path_on_data_acked_) {
    QUIC_FLAG_COUNT(gfe2_reloadable_flag_quic_fast_path_on_stream_data_acked);
  }
}
...
bool QuicStreamSendBuffer::WriteStreamDataWithIndex(QuicStreamOffset offset,
  ...
  if (write_index_ != -1) {
    ...
    if (offset >= slice_it->offset) {
      ...
      QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_use_write_index, 1, 2);
    } else {
...
bool QuicStreamSendBuffer::OnStreamDataAcked(
  ...
  *newly_acked_length = 0;
  ...
  while (!buffered_slices_.empty() &&
    ...
    if (use_write_index_) {
      QUIC_FLAG_COUNT_N(gfe2_reloadable_flag_quic_use_write_index, 2, 2);
